...
- **`.env` lo carga `loadDotEnv()`** (`process.loadEnvFile`, que es nativo de Node ≥ 24) en `main.ts` y en `data-source.ts`. Nunca sobrescribe variables ya definidas, así que en producción mandan las de PM2 o el entorno.
