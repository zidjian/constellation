...
- **Los e2e fijan `LLM_PROVIDER: 'rules'`** al construir el env: si heredan el `.env` del desarrollador, cada corrida gasta la clave real de Anthropic (unas 20 llamadas). La excepción a propósito es `llm-fallback.e2e-spec.ts`, que usa `claude` con una clave inválida.
