...
export class RulesRationaleWriter implements RationaleWriterPort {
  write(request: RationaleRequest): Promise<{ texts: string[]; by: 'rules' }> {
    return Promise.resolve({ texts: this.texts(request), by: 'rules' });
  }

  texts({ profile, steps, skillNames }: RationaleRequest): string[] {
    const name = (s: string) => skillNames[s] ?? s;
    return steps.map(({ course, reason, dependents }) => {
