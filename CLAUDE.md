...
- **IA con Claude (`LLM_PROVIDER=claude`):** modelo `ANTHROPIC_MODEL` (por defecto `claude-opus-5`), salida estructurada (`output_config.format` con JSON Schema y enum de slugs del catálogo), `effort: low`, `fallbacks: "default"` (beta `server-side-fallback-2026-07-01`) ante rechazos, y sin reintentos del SDK.
  - Cualquier fallo (red, timeout de `LLM_TIMEOUT_MS`, rechazo, JSON inválido o fuera de esquema) pasa a reglas (`withFallback`), y `interpreted_by`/`generated_by` guardan quién respondió de verdad.
  - **Intérprete híbrido:** los niveles medidos (retos y autoevaluación) mandan. Claude solo aporta objetivos del texto libre u oferta y niveles de skills que no se midieron.
  - **Redactor del porqué:** arranca en paralelo a la emisión de pasos. Si falta o sobra una posición, todo se redacta por reglas.
